...
若在 Windows 上选择了「为所有用户安装 Node.js」请以管理员权限打开一个命令行窗口再安装；若在 Linux/Mac 上将 Node.js 安装到了整个系统，请使用 `sudo npm install -g avoscloud-code` 安装。
...
请注意，这个命令将部署本地源码到远程平台的预备环境（体验版无预备环境则直接部署到生产环境），无条件覆盖之前的版本（无论是从 Git 仓库部署或者还是本地部署）。
...
2016-05-16 16:00:33 [INFO] 开始部署 20160516-155914 到 staging1
...
可以通过 `lean instance list` 命令查询当前生产环境和预备环境的部署状态：
...
2016-05-16 16:03:53 [PROD] [INFO] > jysperm-leanengine-playground@1.0.0 start /home/leanengine/app
...
`avoscloud-code` 本身是开源，基于 [GNU LGPL](https://www.gnu.org/licenses/lgpl.html) 协议，源码托管在 Github: [https://github.com/leancloud/avoscloud-code-command](https://github.com/leancloud/avoscloud-code-command)，欢迎大家贡献。
