...
{% set sdk_version = "{{sdkversion.android}}" %}
...
dependencies {
    ...
    // LeanCloud 基础包
    compile ('cn.leancloud.android:avoscloud-sdk:{{sdk_version}}')

    // 推送与实时聊天需要的包
    compile ('cn.leancloud.android:avoscloud-push:{{sdk_version}}@aar'){transitive = true}

    // LeanCloud 统计包
    compile ('cn.leancloud.android:avoscloud-statistics:{{sdk_version}}')

    // LeanCloud 用户反馈包
    compile ('cn.leancloud.android:avoscloud-feedback:{{sdk_version}}@aar')

    // avoscloud-sns：LeanCloud 第三方登录包
    compile ('cn.leancloud.android:avoscloud-sns:{{sdk_version}}@aar')
    compile ('cn.leancloud.android:qq-sdk:1.6.1-leancloud')
    // 目前新浪微博官方只提供 jar 包的集成方式
    // 请手动下载新浪微博 SDK 的 jar 包，将其放在 libs 目录下进行集成

    // LeanCloud 应用内搜索包
    compile ('cn.leancloud.android:avoscloud-search:{{sdk_version}}@aar')
}
```
