...
* [LeanDB Redis 使用指南](leancache_guide.html)：云引擎应用可以访问 LeanDB Redis　这一高性能、高可用的 Key-Value 内存存储，可以在一些特殊场景（如秒杀、抢红包等）为应用提供很好的性能表现；或者对读写比例很高的数据做缓存，减少对存储服务的压力，提高应用性能表现。
