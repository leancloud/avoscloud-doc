...
3. 访问云引擎自定义域名的请求不支持 WebSocket 和 HTTP PATCH 方法，请求超时限制为 10 秒。商用版用户如有需要可以配置独立 IP 后使用 A 记录绑定。
4. 自定义域名使用 CNAME 配置域名解析，因此不支持绑定裸域名，以免影响该域名下的其他域名的解析以及与 MX 等记录冲突。商用版用户如果需要使用裸域名作为 API 域名或云引擎域名可以配置独立 IP 后使用 A 记录绑定。
5. 配置独立 IP 后使用 A 记录绑定的域名使用独立入口，没有静态资源加速功能，不过开发者可以[接入第三方 CDN](cdn-guide.html)。
6. 一个应用（文件域名、API 域名）或分组（云引擎域名）上可以绑定多个域名，但文件服务同一时间只能选择使用一个域名（可以在控制台切换）。
7. 国际版只支持绑定云引擎自定义域名，不支持绑定文件域名和 API 域名。国际版的云引擎自定义域名支持 WebSocket 请求和 PATCH 请求，但不支持静态资源加速。如果希望在国际版云引擎绑定裸域名，建议选择支持支持 ANAME 或 CNAME Flattening 记录的域名服务商。另外，国际版可以绑定未备案的域名。
...
共享域名有很多应用共同使用，因此风险较大。
我们强烈建议开发者推动用户升级到使用自定义域名的新版本客户端应用，以免业务受到不必要的影响。
...
未配置独立 IP 的情况下，绑定域名均需要设置 CNAME，因此这里我们简单解释下如何设置 CNAME。
