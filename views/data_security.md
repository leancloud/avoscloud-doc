...
万一发生泄露，请立即前往「控制台 > 设置 > 应用凭证 > Credentials」重置 MasterKey。
...
**控制台 > 存储 > 文件 > 设置** 可以限制用户上传的文件类型，详见[控制台使用指南](dashboard_guide.html#文件设置)的说明。
...
应用每日自动备份，LeanCloud 会保留最近 7 天的备份。
商用版应用可以在 **控制台 > 存储 > 导入导出 > 备份恢复** 恢复最近 7 天的数据（还可以指定需要恢复的 Class 或 objectId）。
所有应用都可以在 **控制台 > 存储 > 导入导出 > 备份导出** 下载备份（可以指定需要下载的备份日期、Class）。 
