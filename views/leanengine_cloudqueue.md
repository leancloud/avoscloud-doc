...
云队列实际上运行在云引擎之外，它通过 HTTP 接受入队等操作，同时也通过 HTTP 来调用云引擎容器中的云函数来完成实际的任务执行，我们的 SDK 会封装这些细节。云队列的入队接口（`Cloud.enqueue`）目前仅限在云引擎内使用 masterKey 权限调用（后续可能会开放客户端调用，见 [后续计划](#后续计划)）；结果查询接口（`Cloud.getTaskInfo`）则允许客户端直接使用 uniqueId 调用。
...
- **定时任务** 现在定时任务是云队列的一个子功能，你可以在控制台上创建和管理任意数量的定时任务，可以使用 [CORN 表达式](#CORN 表达式) 或设置间隔时间。
- **并发控制** 云队列会将入队的任务暂存起来，以 1 个并发的速度逐步地执行，避免云引擎实例过载（后续我们会引入更智能的并发调节算法，见 [后续计划](#后续计划)）。
...
Cloud.define('closeOrder', async function({params}) {
  try {
    ...
    await order.save({status: 'closed'})
  } catch (err) {
...
目前我们只在 Node SDK（3.2 以上版本）添加了实验性的云队列支持：
...
- **仅限云队列调用的云函数** 我们正在为 `Cloud.define` 添加一个 `internal` 选项，通过这种方式定义的云函数只能被 Cloud Queue 或其他具有 masterKey 权限的代码调用。
...
- **与多分组配合使用** 我们我们只有主要分组支持云函数，但你可以在任意分组来仅限入队操作（`Cloud.enqueue`），最终都会执行到主要分组上的云函数。同时我们有计划在所有分组上都支持云函数，但优先级较低。
