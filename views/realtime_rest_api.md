...
msgid | 可选 | 起始的消息 id，**使用时必填加上对应消息的时间戳 max_ts 参数，一起作为查询的起点**。
...
签名采用 Hmac-sha1 算法，输出字节流的十六进制字符串 (hex dump)，签名的 key 必填是应用的 master key，签名的消息格式如下：
...
删除一条指定的聊天历史记录，必填采用 master key 授权，所以不建议在客户端使用此接口。
