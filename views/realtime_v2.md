...
**m**|members|Array||普通对话的所有参与者（仅针对普通对话，暂态对话和服务号并不支持持久化的成员列表）
...
对于应用中存在系统帐号的场景，我们建议您通过下文提到的[服务号](#普通对话_Normal_Conversation_)来实现，以避免对单一帐号创建过多的对话影响您应用的性能。
...
#### 服务号（System Conversation）

这是用于实现机器人、公众号、服务账号等场景的对话，也可以用作发送应用内通知的通道。这种对话具有以下特点：

* 加入即订阅，离开即退订，支持无限的订阅人数
* 开发者可以通过 REST API 以服务号的渠道给所有用户、订阅用户或指定用户（不要求用户订阅）发消息
* 用户可以给服务号发消息，消息和相关信息会存储在数据存储中的 `_SysMessage` 表，并不会被其他订阅用户收到
* 开发者可以配置 [Web Hook](#Web_Hook) 地址接收用户发给服务号的消息，并利用 REST API 发消息回复
* 在 SDK 层面，服务号的接口与普通对话完全一致
* 在 `_Conversation` 表中，以 `sys` 为 `true` 来标记（`m`列在服务号中将被忽略）
...
暂态对话和服务号通常和应用中的特定实体绑定，可以通过 REST API 提前创建，通过应用中的业务逻辑
把对话 ID 下发给最终用户。
...
发送消息时，开发者可以指定消息的发送者、对话 ID，对于服务号还可以指定消息的接收者。
...
根据政策的要求，我们为多人的普通对话、暂态对话和服务号进行敏感词过滤。海外节点暂未启用。
...
* 广播消息必须与服务号关联，广播消息和一般的服务号消息会混合在服务号的记录中
...
服务号可以用于实现机器人自动回复、公众号、服务账号等功能。在我们的 [官方聊天 Demo](http://leancloud.github.io/leanmessage-demo/) 中就有一个使用服务号 hook 实现的机器人 MathBot，它能计算用户发送来的数学表达式并返回结果，[其服务端源码](https://github.com/leancloud/leanmessage-demo/tree/master/server) 可以从 GitHub 上获取。

### 服务号的创建

服务号也是对话的一种，创建后也是在 `_Conversation` 表中增加一条记录，只是该记录 `sys` 列的值为 true，从而与普通会话进行区别。具体创建方法请参考: [创建服务号](#创建服务号) 。

### 服务号消息的发送

服务号给用户发消息请参考： [REST API - 给部分订阅者发消息](realtime_rest_api_v2.html#给部分订阅者发消息)。
用户给服务号发送消息跟用户给普通对话发消息方法一致。

您还可以利用服务号发送广播消息给全部用户。相比遍历所有用户 ID 逐个发送，广播消息只需要调用一次 REST API。
关于广播消息的详细特征可以参考[消息 - 广播消息](#广播消息)。

### 获取服务号消息记录

获取服务号给用户发送的消息记录请参考： [查询服务号给特定订阅者发的消息](realtime_rest_api_v2.html#查询服务号给特定订阅者发的消息)

获取用户给服务号发送的消息记录有以下两种方式实现：
- `_SysMessage` 表方式，在应用首次有用户发送消息给某服务号时自动创建，创建后我们将所有由用户发送到服务号的消息都存储在该表中。
- [Web Hook](#Web_Hook) 方式，这种方式需要开发者自行定义 [Web Hook](#Web_Hook)，用于实时接收用户发给服务号的消息。

### 服务号消息结构

#### `_SysMessage`

存储用户发给服务号的消息，各字段含义如下：

字段 | 说明
--- | ---
ackAt | 消息送达的时间
bin | 是否为二进制类型消息
conv | 消息关联的服务号 Pointer
data | 消息内容
...
#### Web Hook
{% if node=='qcloud' %}
需要开发者自行在 `控制台> **消息** > **实时消息** > **设置** > **服务号消息回调设置**`定义，来实时接收用户发给服务号的消息，消息的数据结构与上文所述的 `_SysMessage` 一致。
{% else %}
需要开发者自行在 [控制台> **消息** > **实时消息** > **设置** > **服务号消息回调设置**](/messaging.html?appid={{appid}}#/message/realtime/conf) 定义，来实时接收用户发给服务号的消息，消息的数据结构与上文所述的 `_SysMessage` 一致。
{% endif %}

当有用户向服务号发送消息时，我们会通过 HTTP POST 请求将 JSON 格式的数据发送到用户设置的 Web Hook 上。请注意，我们调用 Web Hook 时并不是一次调用只发送一条消息，而是会以批量的形式将消息发送过去。从下面的发送消息格式中能看到，JSON 的最外层是个 Array。
