...
在某些使用场景下，您可能会希望能创建出按某些条件满足唯一性的对话，比如用户 A 创建一个成员为 A B 的对话，而 B 可能创建一个成员为 B A 的对话，您可能会希望这创建出来的两个对话是相同的。

1. 需要在 _Conversation 表中新增一个字段 uniqueId，并给该字段添加唯一索引；
2. 每次创建对话前，根据您的需求为该对话创建一个唯一 id。比如您认为成员是 A、B 的对话和成员是 B、A 的对话是同一个对话，可以将成员列表进行排序并通过 MD5 计算出代表该对话的唯一 id。
3. 在创建对话时，以 uniqueId 为键，以前一步计算出来的唯一 id 为值，放入新创建对话的 attribute 内，我们就会
