...
「对话权限」功能作为实时通信的一项补充，可以将对话内成员划分成不同角色，实现类似 QQ 群管理员的效果。使用这个功能需要在控制台开启「对话权限」功能。
...
需要注意一点，Owner 对应 _Conversation 表的 c 字段，如需变更 Owner，直接使用「更新对话的 API」 修改该字段即可，无需使用对话权限 API 操作。
