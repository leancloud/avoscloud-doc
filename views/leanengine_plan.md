...
每个分组都可以部署云函数、Hook 和定时任务。但如果当前部署代码中部分云函数与其他组的同名时，默认情况会提示错误并中断部署，防止意外重复定义云函数。当然也可以通过在部署时指定 `overwriteFunctions` 参数强制替换其他组云函数的实现。我们建议强制替换后尽快移除不需要的云函数，否则每次部署都需要额外增加参数，并且重复定义的云函数也不易于理解和维护。
