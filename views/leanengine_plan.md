...
这篇文档主要介绍云引擎的付费方案、如何管理云引擎的资源、如何使用多分组功能。
...
对于商业项目和正式上线的产品，开发者可根据需要付费购买更多不同规格的标准版实例，标准实例 24 小时运行随时待命。同时还提供了负载均衡、分组管理、预备环境、平滑部署等管理功能，充分保障服务的可用性：
...
- 可以 [创建多个分组](#分组管理)，部署不同代码（甚至不同的语言），绑定不同的域名。
...
开发者还可以使用云引擎的多分组功能，建立多个独立的云引擎实例分组，在访问同一数据源的情况下，部署多套不同的服务器端业务代码，并对每个分组设置独立的二级域名，实现更丰富的业务需求：
...
关于分组管理功能费用请参考 [分组管理功能价格](#分组管理功能价格) 。
...
如果应用云引擎只有一个分组，则不收取费用。

如果应用云引擎分组大于一个，则认为开启多分组功能，价格为当前应用下所有基本资源的数量 ✕ 基本资源单价 ✕ 20%。
...
如果应用移除其他分组，只保留一个，则认为关闭多分组功能，从次日开始不收取费用。
