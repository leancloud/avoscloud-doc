...
开发者可以利用这两个 Hook 函数，结合 LeanDB Redis 来完成一组客户端实时状态查询的 endpoint，具体可以参考文档[即时通讯中的在线状态查询](realtime-guide-onoff-status.html)。
...
例如，创建对话后，将对话的 ID 存储到 Redis 的最近创建对话列表：
...
例如，如果用户自行离开了对话，那么将这个对话的 ID 存储到 Redis（应用可以利用这些数据实现展示「最近离开的对话」乃至重新加入的功能）：
...
例如，客户端上线后更新 Redis，供查询客户端的实时在线状态：
...
例如，客户端下线后更新 Redis，供查询客户端的实时在线状态：
...
[即时通讯中的在线状态查询](realtime-guide-onoff-status.html) 提供了完整的 Node.js 示例（包括 LeanDB Redis 连接，久未上线的客户端清理，配套的返回在线状态的云函数，以及如何在客户端调用），可以参考。
